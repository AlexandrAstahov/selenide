...
public class CardDeliveryOrder {
    ...
    public void successfulSending() {
        ...
        $("[data-test-id='phone'] input").val("+79160147854");
    ...
    void dateCheck() {
        $x("//input[@placeholder=\"Город\"]").val("Кызыл");
        $x("//input[@type=\"tel\"]").doubleClick().sendKeys(Keys.DELETE);
        $x("//input[@placeholder=\"Дата встречи\"]").val(calendar(2));
        $("[data-test-id='name'] input").val("Астахов Александр");
        $("[data-test-id='phone'] input").val("+79160147854");
        $("[data-test-id='agreement']").click();
        $x("//*[contains(text(),'Забронировать')]").click();
    ...
    void fieldValidationFirstAndLastName() {
        ...
        $x("//input[@type=\"tel\"]").doubleClick().sendKeys(Keys.DELETE);
        $x("//input[@placeholder=\"Дата встречи\"]").val(calendar(3));
        $("[data-test-id='name'] input").val("Astahov Alexandr");
        $("[data-test-id='phone'] input").val("+79160147854");
        $("[data-test-id='agreement']").click();
        $x("//*[contains(text(),'Забронировать')]").click();
    ...
    void phoneFieldCheck() {
        ...
        $x("//input[@placeholder=\"Дата встречи\"]").val(calendar(3));
        $("[data-test-id='name'] input").val("Астахов Александр");
        $("[data-test-id='phone'] input").val("+7916014785");
        $("[data-test-id='agreement']").click();
        $x("//*[contains(text(),'Забронировать')]").click();
